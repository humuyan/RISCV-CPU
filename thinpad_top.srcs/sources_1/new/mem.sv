...
assign done = (state == S_DONE || // done processing uart
              ((state == S_READ_RAM || state == S_WRITE_RAM) && 
              (next_state != S_READ_UART && next_state != S_WRITE_UART)) // continuously processing RAM
              ) ? 1'b1 : 1'b0;
...
always_comb begin
    ...
    cur_data_in = data_in;
    case(state)
        S_IDLE, S_READ_RAM, S_WRITE_RAM: begin
        ...
        S_READ_UART_GAP: begin
            ...
            next_state = S_READ_UART_GAP_2;  
        end
        S_READ_UART_GAP_2: begin
            next_state = S_DONE;
        end
        S_WRITE_UART: next_state = S_WRITE_UART_WAIT_TBRE;
        S_WRITE_UART_WAIT_TBRE: begin
            ram_enable = IDLE;
            if (uart_tbre == 1) next_state = S_WRITE_UART_WAIT_TSRE; 
            else next_state = S_WRITE_UART_WAIT_TBRE;
        end
        S_WRITE_UART_WAIT_TSRE: begin
            if (uart_tsre == 1) next_state = S_DONE;
            else next_state = S_WRITE_UART_WAIT_TSRE;
        end
        S_READ_UART_STATE: next_state = S_DONE;
        S_DONE: begin
            ram_enable = IDLE;
            next_state = S_IDLE;
        end
